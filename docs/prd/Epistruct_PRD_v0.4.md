...
```
원본 노드(그룹/공유) → AI 번역 제안(대상 맥락으로 변환, 보조)
  → 사용자 이해·정리(필수 게이트: 직접 수정·재구성) → 대상 스코프에 새 노드 생성
```
...
### Phase 2 — 공유 + 지식 융합 (다른 두뇌와 연결)

- F11. 스코프 부여/변경: 모든 노드(P/C/M/D)에 개인/그룹/공유 스코프 설정.
- F12. 스코프 간 이동(가져오기/올리기): 개인↔그룹↔공유. 항상 5.5의 이해 게이트를 경유.
- F13. AI 지식 번역: 원본 노드를 대상 스코프의 맥락으로 변환 제안(게이트의 보조 단계).
- F14. 충돌·중복 감지 + 병합 제안: 가져온 노드가 대상 스코프의 기존 그래프와 중복·상충할 때.
- F15. 출처(provenance) 추적: 이동된 노드의 계보·재구성 이력 기록(Phase 3 기여도·격차의 기반).
- F16. 그룹 공간 운영: 그룹 생성·멤버십·그룹 스코프 노드의 공동 관리.
...
**S3. 스코프 간 융합**
그룹 스코프에 동료가 올린 `C:transaction-isolation` 노드를 사용자가 자기 개인 스코프로 가져온다 → AI가 사용자의 기존 "동시성" 맥락에 맞게 번역 제안 → 사용자가 직접 수정·정리해 새 노드로 확정한다. 원본과는 출처 계보로 연결되며, 사용자의 그래프는 항상 사용자의 언어로 유지된다.
...
- **동시성**: 그룹 스코프 노드는 여러 멤버가 동시에 편집할 수 있다 → 충돌을 안전하게 해결(낙관적 락 / 버전 관리 / 병합 제안). 공유 스코프는 거버넌스 규칙 필요.
